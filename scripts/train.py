...
def _resolve_num_views(model_settings) -> int:
    slot_counts: list[int] = []
    for stream in model_settings.streams:
        if stream.backbones:
            slot_counts.append(len(stream.backbones))
        elif stream.repeat:
...
def main() -> None:
    ...
    if config.model.per_backbone_views:
        train_transform = MultiViewTransform(train_transform, _resolve_num_views(config.model))
    ...
    degraded_eval_transform = None
    if config.augmentation.static_val_augmentation:
        degraded_eval_transform = build_eval_transform(
            image_size=config.data.image_size,
            mean=config.augmentation.mean,
            std=config.augmentation.std,
            static_augmentation=True,
    ...
    train_collate = (
        MultiResolutionBatchCollate(config.data.train_image_sizes)
        if config.data.train_image_sizes
        else None
    )
    ...
    degraded_val_loader = None
    if degraded_val_dataset is not None:
        degraded_val_sampler = (
            DistributedSampler(degraded_val_dataset, shuffle=False) if is_distributed() else None
        )
        degraded_val_loader = DataLoader(
            degraded_val_dataset,
            batch_size=config.data.batch_size,
            shuffle=False,
            sampler=degraded_val_sampler,
            num_workers=config.data.num_workers,
            pin_memory=config.data.pin_memory,
            persistent_workers=config.data.persistent_workers and config.data.num_workers > 0,
        )
    ...
    trainer = Trainer(
        ...
        degraded_val_loader=degraded_val_loader,
