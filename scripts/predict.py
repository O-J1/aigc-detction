...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("none", "hflip"),
        default="none",
        help="Test-time augmentation: average fused probability over augmented views.",
    )
    ...
    with output_path.open("w", newline="", encoding="utf-8") as output_file:
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                fused_probabilities = outputs["fused_prob"].detach()
                if args.tta == "hflip":
                    flipped_outputs = model(torch.flip(images, dims=[-1]))
                    fused_probabilities = 0.5 * (
                        fused_probabilities + flipped_outputs["fused_prob"].detach()
                    )
                probabilities = fused_probabilities.cpu().flatten().tolist()
