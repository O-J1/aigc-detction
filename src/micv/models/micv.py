...
class MICVStream(nn.Module):
    ...
    def forward(self, images: Tensor) -> Tensor:
        if images.ndim == 5:
            # [B, num_views, C, H, W]: feed each committee slot its own
            # augmented view (extra views are ignored, missing views reuse the
            # last one).
            num_views = images.shape[1]
            slot_features = [
                backbone(images[:, min(slot_index, num_views - 1)])
                for slot_index, backbone in enumerate(self.backbones)
            ]
        else:
...
class MICVDualStreamEnsemble(nn.Module):
    ...
    def forward(self, images: Tensor) -> dict[str, Tensor]:
        ...
        stream1_prob = torch.sigmoid(stream1_logits.float())
        stream2_prob = torch.sigmoid(stream2_logits.float())
